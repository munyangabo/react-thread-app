...
const Thread = ({user, setOpenPopUp,filteredThread, getThreads,setFilteredThread}) => {
  const timePassed = moment().startOf('day').fromNow(filteredThread.timestamp)
  const handleClick = () => {
    setOpenPopUp(true)
    setFilteredThread(filteredThread)
  }

  console.log('filteredThread', filteredThread)
  const postlike = async() => {
    const hasBeenLikedByUseer = 
    filteredThread.likes.some( like => like.user_uuid === user.user_uuid)
    if (!hasBeenLikedByUseer) {
